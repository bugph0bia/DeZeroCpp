...
- numrical_diff 関数
    ...
    - …と思ったら、合成関数の微分のところでに、通常の関数 `f` をこの関数に渡すコードが出てきてしまい、そもそも Function 型にしてはダメだと気付く。 `std::function` を使用して、Callable なオブジェクトを受け取れるように変更した。C++ は型に厳密なので、呼び出す対象の関数型を `Varialbe(Variable)` に限定せざるを得なかったが、この先、Python のダックタイピングをどこまで受け入れられるか不安。
- step04
